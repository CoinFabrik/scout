        ...
        pub fn deposit(&mut self, from: AccountId) -> Result<(), Error>{
            if self.env().caller() != self.buyer {
                ...
                return res.map_err(|err| match err {
                    ob_PSP22Error::Custom(err_msg) => Error::PSP22Error(PSP22Error::Custom(err_msg)),
                    ob_PSP22Error::InsufficientBalance => Error::PSP22Error(PSP22Error::InsufficientBalance),
                    ob_PSP22Error::InsufficientAllowance => Error::PSP22Error(PSP22Error::InsufficientAllowance),
                    ob_PSP22Error::ZeroRecipientAddress => Error::PSP22Error(PSP22Error::ZeroRecipientAddress),
                    ob_PSP22Error::ZeroSenderAddress => Error::PSP22Error(PSP22Error::ZeroSenderAddress),
                    ob_PSP22Error::SafeTransferCheckFailed(err_msg) => Error::PSP22Error(PSP22Error::SafeTransferCheckFailed(err_msg))
                });
            }
    ...
    mod e2e_tests {
        ...
        async fn e2e_test(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let deposit = build_message::<UnrestrictedTransferFromRef>(escrow_account_id.clone())
                .call(|contract| contract.deposit(ink_e2e::account_id(ink_e2e::AccountKeyring::Bob)));
